...
from metrics import *
from utils import *
import nets.Config as config
from torch.utils.tensorboard import SummaryWriter
from nets.U_SCIFNet import U_SCIFNet

parser = argparse.ArgumentParser(description="PyTorch BasicIRSTD train")
parser.add_argument("--model_names", default=['U_SCIFNet'], type=list, help="'ACM', 'ALCNet', 'DNANet', 'ISNet', 'UIUNet', 'RDIAN', 'RISTDnet'")
...
parser.add_argument("--every_save_pth", default=650, type=int)
...
parser.add_argument("--save", default=r'./log/git_NUDT', type=str, help="Save path of checkpoints")
parser.add_argument("--log_dir", type=str, default="./otherlogs/git_NUDT", help='path of log files')
...
config_vit = config.get_SCIF_config()
...
def train():
    ...
    for idx_epoch in range(epoch_state, opt.nEpochs):
        ...
        if (idx_epoch + 1) % opt.every_print == 0:
            ...
            opt.f.write(time.ctime()[4:-5] + ' Epoch---%d, total_loss---%f,lr---%f,\n'
                        % (idx_epoch + 1, total_loss_list[-1],scheduler.get_last_lr()[0]))
            total_loss_epoch = []
        ...
        if (idx_epoch + 1) >= opt.begin_test and (idx_epoch + 1) % opt.every_test == 0:
            ...
            with torch.no_grad():
                eval_mIoU = mIoU()
                eval_PD_FA = PD_FA()
                binary_metric = Binary_metric(thr=opt.threshold)
                test_loss = []
                for idx_iter, (img, gt_mask, size, _) in enumerate(test_loader):
                    ...
                    binary_metric.update(pred, gt_mask)
                test_loss.append(float(np.array(test_loss).mean()))
                results1 = eval_mIoU.get()
                results2 = eval_PD_FA.get()

                mean_results = binary_metric.get_mean_result()  # mean IoU and F1 scores over all test batches
                norm_results = binary_metric.get_norm_result()  # normalized IoU and F1 scores
                writer.add_scalar('mIoU', results1[-1], idx_epoch + 1)
                writer.add_scalar('testloss', test_loss[-1], idx_epoch + 1)

                writer.add_scalar('nIoU', norm_results['iou'], idx_epoch + 1)  # 记录 IoU (mean)
                writer.add_scalar('F1', norm_results['fscore'], idx_epoch + 1)  # 记录 F1 (mean)

                writer.add_scalar('PD', results2[0], idx_epoch + 1)  # Final_PD
                writer.add_scalar('FA', results2[1], idx_epoch + 1)  # Final_FA
        ...
        if results1[1] > best_mIOU[1]:
            best_mIOU = results1
            best_Pd = results2
            best_F1 = mean_results['fscore']  # 更新最佳F1分数
            best_norm_IoU = norm_results['iou']  # 更新最佳nIoU分数
...
class Net(nn.Module):
    def __init__(self, model_name, mode):
        ...
        if model_name == 'U_SCIFNet':
            if mode == 'train':
                self.model = U_SCIFNet(config_vit, mode='train', deepsuper=True)
            else:
                self.model = U_SCIFNet(config_vit, mode='test', deepsuper=True)
