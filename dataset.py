...
class TrainSetLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(mask.shape) > 2:
            mask = mask[:, :, 0]
        img_patch, mask_patch = random_crop(img, mask, self.patch_size, pos_prob=0.5)  # output:256 256

        img_patch, mask_patch = self.tranform(img_patch, mask_patch)  # data augmentation through flipping
        img_patch, mask_patch = img_patch[np.newaxis, :], mask_patch[np.newaxis, :]  # increase the dimensionality
        img_patch = torch.from_numpy(np.ascontiguousarray(img_patch))  # numpy to tensor
        mask_patch = torch.from_numpy(np.ascontiguousarray(mask_patch))  # numpy to tensor
...
class TrainSetLoader04(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        rnd_bn = np.random.normal(0, 0.03)
        img += rnd_bn
