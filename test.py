...
def cal_tp_pos_fp_neg(output, target, nclass, score_thresh):
    ...
    if len(target.shape) == 3:
        print('？？？？')  # Add a dimension to make the size of the target match the output size.
...
parser.add_argument("--model_names", default=['U_SCIFNet'], type=list,
                    help="model_name: 'ACM', 'Ours01', 'DNANet', 'ISNet', 'ACMNet', 'Ours01', 'ISTDU-Net', 'U-Net', 'RISTDnet'")
parser.add_argument("--pth_dirs", default=['NUDT-SIRST/NUDT_best.tar'], type=list)
parser.add_argument("--dataset_dir", default='/root/autodl-tmp/U_SCIFNet/datasets', type=str, help="train_dataset_dir")
parser.add_argument("--dataset_names", default=['NUDT-SIRST'], type=list)
parser.add_argument("--utilss", default=['NUAA-SIRST', 'NUDT-SIRST', 'IRSTD-1K'], type=list,
                    help="dataset_name: 'NUAA-SIRST', 'NUDT-SIRST', 'IRSTD-1K', 'SIRST3', 'NUDT-SIRST-Sea'")
parser.add_argument("--img_norm_cfg", default=None, type=dict,
                    help="specific a img_norm_cfg, default=None (using img_norm_cfg values of each dataset)")
parser.add_argument("--save_img", default=True, type=bool, help="save image of or not")
parser.add_argument("--save_img_dir", type=str, default='/root/autodl-tmp/U_SCIFNet/Result/',
                    help="path of saved image")
parser.add_argument("--save_log", type=str, default='/root/autodl-tmp/U_SCIFNet/log/', help="path of saved .pth")
...
def test():
    test_set = TestSetLoader(opt.dataset_dir, opt.train_dataset_name,
                             opt.test_dataset_name, opt.img_norm_cfg)
    ...
    net = U_SCIFNet(config_vit, mode='test', deepsuper=True).cuda()
